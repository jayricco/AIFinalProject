...
def train(args, model):
    ...
    valid_sample = utils.resample(valid_set, replace=False, n_samples=700)
    data_pre = []
    target_pre = []
    ...
    data = data.view(700, 1, num_features)
    target = target.view(700, 1)
    ...
    test_epoch(model, train_set, test_set, test_sizes=[10, 100, 1000, 2000, 5000], test_ks=[-1, 2, 5, 10, 50])
    te_fi.close()
...
def train_epoch(args, model, train_set, valid_set, optimizer, criterion):
    ...
    while True:
        ...
        fig.savefig(dir_name + "img2/" + fname, dpi=125, transparent=True)
        files.append(fname)

        print("[%d] Finished With Graph Render!" % mypid)

        dataset = utils.resample(train_set, replace=False, n_samples=train_batch + bp_batch)
        data_pre = []
        target_pre = []
        for d in dataset:
            data_pre.append(torch.from_numpy(np.asarray([d[0]], dtype=np.float32)))
        ...
        print("[%d] Tree Training Complete! (Time: %f sec, # Nodes: %d)\nStarting Backpropagation Step..." % (
        mypid, training_time, model.count))
