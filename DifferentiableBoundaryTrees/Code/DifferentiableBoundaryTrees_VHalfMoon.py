...
if __name__ == "__main__":
    ...
    for current_k in [3, 5, 10, 20, 50, 100]:
        ...
        loss_fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(15, 5))
        ...
        while True:
            ...
            if i == 0 or i % 100 == 0:
                ...
                ax2.clear()
                ax3.clear()
                ax1.scatter(x[:, 0], x[:, 1], marker='+', c=y, linewidth=0.5, cmap=color.ListedColormap(['b', 'r']))
                ax1.set_title("Original Input Space")
                ax2.scatter(x_t[:, 0], x_t[:, 1], marker='+', c=y, linewidth=0.5, cmap=color.ListedColormap(['b', 'r']))
                ...
                nx.draw_networkx(dbt.graph, pos=p, node_color=dbt.n_colors, cmap=color.ListedColormap(['b', 'r']), vmin=0.0,
                                 vmax=1.0)
            ...
            if np.abs(avg_loss_last - avg_loss) < 0.001 and np.abs(avg_loss) <= 0.01:
                break
        ...
        for n_e in [10, 50, 100, 1000]:
            ...
            ax2.clear()
            ax3.clear()
            ax1.scatter(x[:, 0], x[:, 1], marker='x', c=y, s=10, linewidth=0.5, cmap=color.ListedColormap(['b', 'r']))
            ax1.set_title("Original")
            ax2.scatter(x_t[:, 0], x_t[:, 1], marker='x', c=y, s=10, linewidth=0.5, cmap=color.ListedColormap(['b', 'r']))
