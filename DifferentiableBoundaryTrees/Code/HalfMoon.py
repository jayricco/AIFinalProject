from __future__ import print_function
import torch
import torch.nn as nn
from torch.autograd import Variable
import numpy as np
import os
import csv
import sklearn.datasets as ds
import sklearn.utils as utils
import time
import copy
import random

num_classes = 2
num_features = 2
num_outputs = 2
train_batch = 100
bp_batch = 50
current_k = -1
dir_name = './DBT_HALFMOON/'
time_str = time.strftime("%Y%m%d-%H%M%S")
attr_str = "data_file-tb_"+str(train_batch)+"-bpb_"+str(bp_batch)+"-"
format_str = '.csv'
f_name_tr = dir_name + attr_str + time_str + "_TRAIN" + format_str
f_name_te = dir_name + attr_str + time_str + "_TEST" + format_str
tr_fi = open(f_name_tr, 'w+b')
te_fi = open(f_name_te, 'w+b')
train_data = {"K": [], "Iter": [],  "TrainTime": [], "Nodes": [], "BackPropTime": [], "Loss": []}#, "TestTime": [], "Accuracy": []}
test_data = {"K": [], "NumTrain": [], "TrainTime": [], "Nodes": [], "TestTime": [], "Accuracy": []}

train_writer = csv.DictWriter(tr_fi, train_data.keys())
test_writer = csv.DictWriter(te_fi, test_data.keys())

def train(args, model):
    random.seed(932523)
    optimizer = torch.optim.Adam(params=model.parameters(), lr=1e-4)
    criterion = torch.nn.CrossEntropyLoss(size_average=False)

    train_set = ds.make_moons(n_samples=5000)
    train_set = zip(train_set[0], train_set[1])

    test_set = ds.make_moons(n_samples=5000)
    test_set = zip(test_set[0], test_set[1])

    train_writer.writeheader()
    train_epoch(model, train_set, optimizer, criterion)
    tr_fi.close()

    test_writer.writeheader()
    test_epoch(model, train_set, test_set, test_sizes=[10, 100, 1000, 2000, 5000], test_ks=[-1, 2, 10, 25, 50])
    te_fi.close()

def train_epoch(model, train_set, optimizer, criterion):
    i = 1
    loss_track = []
    y_onehot = torch.FloatTensor(1, num_classes)
    avg_loss_last = 0.0
    avg_loss = 0.0
    while True:

        print("Iteration %d " % i)
        print("[%d] Training Tree..." % os.getpid())
        time_s = time.clock()
        r = range(len(train_set))
        random.shuffle(r)
        for (batch_idx, index) in enumerate(r[slice(0, train_batch)]):
            (X, y) = train_set[index]
            X = Variable(torch.from_numpy(np.asarray([X], dtype=np.float32)))
            Y = Variable(torch.from_numpy(np.asarray([y])), requires_grad=False)
            y_oh = np.zeros([1, 2], dtype=np.float32)
            y_oh[0, Y.data[0]] = 1.0
            Y_oh = Variable(torch.from_numpy(y_oh))
            model.train_tree(X, Y, Y_oh)
            if ((batch_idx + 1) % int(np.ceil(train_batch/5.0))) == 0:
                print("Processed Ex: {0}/{1}".format(batch_idx + 1, train_batch))
        time_f = time.clock()
        training_time = time_f - time_s
        print("[%d] Tree Training Complete! (%f sec)" % (os.getpid(), training_time))
        print("[%d] Tree Size: %d (nodes)" % (os.getpid(), model.count))
        print("[%d] Starting Backpropagation Step!" % os.getpid())

        loss_sum = 0.0
        time_s = time.clock()

        for (batch_idx, index) in enumerate(r[slice(train_batch, train_batch+bp_batch)]):
            optimizer.zero_grad()
            (X, y) = train_set[index]
            X = Variable(torch.from_numpy(np.asarray([X], dtype=np.float32)))
            Y = Variable(torch.from_numpy(np.asarray([y])), requires_grad=False)

            out = model.forward(X)
            loss = criterion(out, Y.long()[0])
            loss_sum += loss.data.float()[0]
            loss.backward()
            optimizer.step()
            if ((batch_idx + 1) % int(np.ceil(bp_batch / 5.0))) == 0:
                print("Processed Ex: {0}/{1}".format(batch_idx + 1, bp_batch))
        time_f = time.clock()
        bp_time = time_f - time_s
        print("[%d] Backpropagation Step Complete! (%f sec) " % (os.getpid(), bp_time))
        avg_loss = loss_sum / bp_batch
        print("[%d] Average Loss: %f" % (os.getpid(), avg_loss))


        train_data["K"] = model.k
        train_data["Iter"] = i
        train_data["TrainTime"] = training_time
        train_data["Nodes"] = model.count
        train_data["BackPropTime"] = bp_time
        train_data["Loss"] = avg_loss
        train_writer.writerow(train_data)
        if np.abs(avg_loss_last - avg_loss) <= 0.0001 and avg_loss < 0.01:
            print("Training Finished!\n")
            break
        print("")
        avg_loss_last = avg_loss
        model.clear_tree()
        i += 1

"""print("Starting Validation Test...")
        num_right = 0.0
        for index in tqdm(vset_randomizer[0:1000], desc="Validation Test", total=1000, miniters=50):
            closest_node = dbt.query_tree(X_valid[index])
            y_pred = dbt.data[closest_node][1].data[0]
            if y_pred == Y_valid[index].data[0]:
                num_right += 1.0

        print("Validation Accuracy: %f" % ((num_right/1000.0)*100.0))"""

def test_epoch(model, train_set_master, test_set, **args):
    #train with a specific k, test across all k's
    print("Starting Test Set Evaluation.")
    sizes = args["test_sizes"]
    ks = args["test_ks"]
    for tk in ks:
        model.k = tk
        print("Testing with k = {0}".format(model.k))
        for n_e in sizes:
            model.clear_tree()
            y_onehot = torch.FloatTensor(1, num_classes)

            print("Training tree with {0} examples...".format("all" if n_e >= len(train_set_master) else n_e))

            dataset = utils.resample(train_set_master, replace=False, n_samples=len(train_set_master) if n_e > len(train_set_master) else n_e)
            data_pre = []
            target_pre = []
            for d in dataset:
                data_pre.append(torch.from_numpy(np.asarray([d[0]], dtype=np.float32)))
                target_pre.append(torch.from_numpy(np.asarray([d[1]], dtype=np.int64)))
            data = torch.cat(data_pre, dim=0)
            target = torch.stack(target_pre, dim=0)
            train_set = zip(data, target)
            start_t = time.clock()
            for batch_idx, (data, target) in enumerate(train_set):

                X = data.view(1, num_features)

                Y = target
                y_onehot.zero_()
                y_onehot.scatter_(1, Y.view(1, 1), 1)
                X = Variable(X)
                Y = Variable(Y, requires_grad=False)
                Y_oh = Variable(y_onehot)
                model.train_tree(X, Y, Y_oh)
                if ((batch_idx+1) % int(np.ceil(n_e/5.0))) == 0:
                    print("Processed Ex: {0}/{1}".format(batch_idx+1, len(train_set)))
            finish_t = time.clock()
            test_tt = finish_t - start_t
            print("All Done! (Tree Size: {0}, Time: {1})".format(model.count, test_tt))

            print("Performing Test!")
            num_right = 0.0
            num_total = 0.0
            start_t = time.clock()
            for batch_idx, (data, target) in enumerate(test_set):
                data = torch.from_numpy(np.asarray([data], dtype=np.float32))
                target = torch.from_numpy(np.asarray([target], dtype=np.int64))
                X = Variable(data.view(1, num_features))
                Y = Variable(target, requires_grad=False)
                closest_node = model.query_tree(X)
                y_pred = model.data[closest_node][1]
                if y_pred == Y:
                    num_right += 1.0
                num_total += 1
                if ((batch_idx+1) % 1000) == 0:
                    print("Processed Ex: {0}/{1}".format(batch_idx+1, len(test_set)))
            finish_t = time.clock()
            true_testt = finish_t - start_t
            test_data["K"] = model.k
            test_data["TrainTime"] = test_tt
            test_data["NumTrain"] = n_e
            test_data["Nodes"] = model.count
            test_data["TestTime"] = true_testt
            test_data["Accuracy"] = ((num_right/num_total) * 100.0)
            test_writer.writerow(test_data)
            print("Testing Done! (Final Accuracy: %f, Time: %f)" % (((num_right/num_total) * 100.0), true_testt))
    print("Test Epoch Finished Successfully!")